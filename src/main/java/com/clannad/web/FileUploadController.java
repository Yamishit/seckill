...
import com.clannad.utils.FileUploadUtils;
...
public class FileUploadController {
     ...
     * RequestParam必须是文件的key
     */
    @RequestMapping(value = "fileUpload_multipartFile")
    @ResponseBody
    public String fileUpload_multipartFile(HttpServletRequest request,@RequestParam("test") MultipartFile multipartFile) {
        //调用保存文件的帮助类进行保存文件，并返回文件的相对路径
        String filePath = FileUploadUtils.FilesUpload_transferTo_spring(request, multipartFile, "/files/upload");
     ...
     * MultipartFile multipartFile = multipartFiles.get("file_upload");
     * 文件的key必须与之对应
     */
    ...
    public String fileUpload_CommonsMultipartResolver(HttpServletRequest request) {
        ...
        if (multipartResolver.isMultipart(request)) {
            ...
            while (iter.hasNext()) {
                ...
                //调用保存文件的帮助类进行保存文件，并返回文件的相对路径
                String fileName = FileUploadUtils.FilesUpload_transferTo_spring(request, multipartFile, "/files/upload");
    ...
    public String upFile(HttpServletRequest request, @RequestParam("file_upload") MultipartFile multipartFile){
        String filePath= FileUploadUtils.FilesUpload_stream(request,multipartFile,"/files/upload");
        return "{'TFMark':'true','Msg':'upload success !','filePath':'" + filePath + "'}";
    }
     ...
     * @RequestParam("file_upload") 文件key统一
     */
        ...
        if (multipartFile != null && multipartFile.length > 0) {
            ...
            try {
                for (int i = 0; i < multipartFile.length; i++) {
                    ...
                    //保存文件
                    String fileName = FileUploadUtils.FilesUpload_transferTo_spring(request, file, "/files/upload");
                    System.out.println(fileName);
                }
    ...
    public void fileDownload_servlet(HttpServletRequest request, HttpServletResponse response) {
        FileUploadUtils.FilesDownload_stream(request,response,"/files/download/mst.txt");
    }
}
